...
|---------------|----------|--------------------------------------------------------|
| `code`        | `string` | Identificador unico del error (usado en logs y modal). |
...
### `SERVER_UNREACHABLE`
- **Severidad:** error
- **Causa tecnica:** No se pudo establecer conexion con el servidor backend. Puede ocurrir por: el servidor esta caido, la URL esta mal configurada, problemas de CORS, o la red bloquea la conexion.
- **Mensaje al usuario:** "No pudimos conectar con el servidor. Puede estar en mantenimiento. Intenta de nuevo en un momento."
- **Accion recomendada:** Verificar que el backend este corriendo. En desarrollo, verificar que la URL en `REACT_APP_API_URL` sea correcta.
...
### `LOGIN_NULL_RESPONSE`
- **Severidad:** error
- **Causa tecnica:** La mutacion `login` de GraphQL retorno `null` para el campo `login`. Esto puede indicar que el resolver del backend no encontro al usuario o hubo un error silencioso.
- **Mensaje al usuario:** "No fue posible iniciar sesion. Verifica tus datos o intenta mas tarde."
- **Accion recomendada (dev):** Revisar los logs del backend. Verificar que el resolver `login` maneje correctamente los casos donde el usuario no existe.

---

## Errores de Registro (Sign Up)
...
- **Severidad:** error
- **Causa tecnica:** La mutacion `createUser` retorno `null`. Esto es lo que causa el error `TypeError: Cannot read properties of null (reading 'Rol')` que se veia originalmente. El backend no creo el usuario pero tampoco retorno un error GraphQL explicito.
...
## Errores de Validacion (Frontend)
...
### `GRAPHQL_BAD_INPUT`
- **Severidad:** warning
- **Causa tecnica:** El servidor GraphQL retorno un error con codigo de extension `BAD_USER_INPUT`. Esto indica que los datos enviados no cumplen con las validaciones del esquema.
...
### `UNKNOWN_ERROR`
- **Severidad:** error
- **Causa tecnica:** Error no categorizado. Ocurre cuando la excepcion no coincide con ninguno de los patrones conocidos (sin `networkError`, sin `graphQLErrors`, y `navigator.onLine === true`).
...
2. Documentar el nuevo codigo en este archivo siguiendo el formato existente.
3. Si es necesario, actualizar `resolveErrorFromException()` para detectar el nuevo tipo de error automaticamente.
4. Usar `showError(ERROR_CODES.NUEVO_CODIGO)` en el componente correspondiente.
